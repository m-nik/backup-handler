...
def main():
    ...
    try:
        ...
        # Elasticsearch
        es_selector = None if (not has_any and not selections['all']) else selections['es'] if selections['es'] else ('all' if selections['all'] else None)
        ...
        # MongoDB
        mongo_selector = None if (not has_any and not selections['all']) else selections['mongodb'] if selections['mongodb'] else ('all' if selections['all'] else None)
        ...
        for inst in mongo_instances:
            ...
            if mongodb_result["status"] == "success":
                prometheus_pusher.update_status("mongodb_backup", f"{name}: backup to {mongodb_result['path']} succeeded", 1)
        ...
        # PostgreSQL
        pg_selector = None if (not has_any and not selections['all']) else selections['postgresql'] if selections['postgresql'] else ('all' if selections['all'] else None)
        ...
        # MariaDB
        maria_selector = None if (not has_any and not selections['all']) else selections['mariadb'] if selections['mariadb'] else ('all' if selections['all'] else None)
